...
```mermaid
graph TD;
    User["User"] -->|"POST /api/v1/calculate"| Orchestrator;
    User -->|"GET /api/v1/expressions/:id"| Orchestrator;
    
    Orchestrator["Orchestrator"] -->|"Parse Expression"| Parse;
    Parse["Parse Expression"] -->|"Build Expression Tree"| BuildTree;
    BuildTree["Build Expression Tree"] -->|"Generate Tasks"| GenerateTasks;
    GenerateTasks["Generate Tasks"] -->|"Task Store"| TaskStore;

    TaskStore["Task Store"] -->|"GET /internal/task"| Agent;
    Agent["Agent"] -->|"Execute Task"| ExecuteTask;
    ExecuteTask["Execute Task"] -->|"POST /internal/task"| Orchestrator;

    Orchestrator -->|"Update Expression Status"| TaskStore;

    classDef mainNode fill:#333,stroke:#ccc,stroke-width:2px,font-weight:bold,color:#fff;
    classDef subNode fill:#222,stroke:#777,stroke-width:1px,font-weight:normal,color:#ddd;
    class User,Orchestrator,Agent mainNode;
    class Parse,BuildTree,GenerateTasks,TaskStore,ExecuteTask subNode;
```
...
│   └── server.go              # Локальный HTTP-сервер для раздачи фронтенда
...
```bash
git clone https://github.com/StarwinM/go_final_sprint2
```
